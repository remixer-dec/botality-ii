export default {
  ...
  wrong_repo_format: 'Repo path must end have a slash in it',
  model_installed: '✅ Model installed!',
  model_selected_ok: '✅ Model changed, restart is required to apply changes',
  ...
  ignore_mode: 'A mode in which access-lists are checked. In whitelist mode, only users in whitelist can access the bot, in blacklist, users in blacklist cannot access the bot, when both modes are active, only whitelisted chats are allowed and blacklisted users in them are ignored.',
  ...
  tts_mode: 'In local mode, tts is used in the bot process, in remote, it makes an API request to a remote-server and receives a file, in localhttp mode, it only acts as remote but only sends the path instead of the whole file.',
  ...
  llm_remote_launch_command: 'Terminal command to launch the server',
  ...
  mm_preload_models_on_start: 'If on, models will be pre-loaded when the bot is launched otherwise, loading starts on-demand',
  ...
  mm_management_policy: 'In COUNT mode only after hitting a limit model will be unloaded, in MEMORY mode, when a there is almost no memory left, a model will be unloaded (but there is no estimation of memory footprint for a model that will be loaded instead)',
